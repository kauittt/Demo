...
export default function ModalCustomer({
    ...
    const { t, i18n } = useTranslation(["table", "common", "modal"]);
    ...
    return (
        <div className="fixed inset-0 bg-black bg-opacity-50 flex items-center justify-center">
            <div className="bg-white p-5 rounded relative w-[550px] ">
                ...
                <h1 className="text-4xl m-3 text-left">{t("desc.label")}</h1>
                <Formik
                    initialValues={{
                    ...
                    {(formik) => (
                        <Form className="grid grid-cols-2 gap-[20px] justify-between">
                            {fields?.map((field, index) => {
                                ...
                                const keyPath = "table." + (index + 1);
                                const label = t(keyPath);
                                return (
                                    <FormInput
                                        label={label}
                                        id={field}
